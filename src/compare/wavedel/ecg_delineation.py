...
def my_delineator(ecg, sampling_rate, adjust_rpeak=True):
    markers = {}
...
def visualise_detection(ecg, qrs_off,sampling_rate):
    #p_peaks,p_on, p_off =  detect_pwave(ecg,qrs_on,sampling_rate)  
    t_peaks, t_on, t_off =  detect_twave(ecg,qrs_off,sampling_rate)  
    wt = wvlt.wavelet_decomposition(ecg, len(ecg),sampling_rate)
    ...
    for marker in t_peaks:
        plt.axvline(x=marker, color='k', linestyle='--')
    #for marker in t_off:
    #    plt.axvline(x=marker, color='r', linestyle='--') 
    #plt.xlim(50,2000)
    plt.show()